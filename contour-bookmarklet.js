(function () {
  ...
  var dependencies = ['jquery', 'https://raw.github.com/jgallen23/toc/master/dist/jquery.toc.js'];
  ...
  function main() {
    ...
    // that don't have a 'console' object.
		...
		$("span:contains('Email Link')").click()
		var docId = $("span:contains('https://www.contourhosted.com')").text().match(/\d+$/)[0]
		$("button:contains('Close')").click()
		window.prompt ("Copy docId (Ctrl-C, Enter)", docId);
  ...
  // be used.
  // Note: these must all be reachable via https (and optionally http, for better performance)
  var SPECIAL_DEPENDENCIES = {
    jquery: '//ajax.microsoft.com/ajax/jquery/jquery-1.7.2.min.js'
  };
